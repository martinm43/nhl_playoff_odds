...
int selectDataCallback(void* data, int argc, char** argv, char** /*azColName*/) {
    ...
    game.home_g = std::stoi(argv[2]);
    game.visitor_team_id = std::stoi(argv[3]);
    game.visitor_g = std::stoi(argv[4]);
...
int main() {
    ...
    int rc = sqlite3_open("nhl_data.sqlite", &db);
    if (rc != SQLITE_OK) {
    ...
    for(int year = start_year;year<=end_year;year++){
        ...
        timeinfo.tm_year = year - 1 - 1900; // Year since 1900 (2023)
        ...
        yearendtimeinfo.tm_mon = 4;           // Month (August, 0-based index)
        yearendtimeinfo.tm_mday = 1;         // Day of the month
        ...
        while(endepochTime < yearendepochTime) {
            ...
            // Query the table
            selectquery = "SELECT id, home_team_id, home_g, visitor_team_id, visitor_g,"
            " season_year , game_datetime FROM games where (game_datetime > "+std::to_string(epochTime)+" and game_datetime < "+std::to_string(endepochTime)+") and (home_g > 0 or visitor_g> 0)"; 
            ...
            int game_size_tol = 1; //originally 1 quarter of season
            ...
            for (unsigned long col=0;col<games.size();col++) {
                ...
                away = games[col].visitor_team_id;
                homescore = games[col].home_g;
                awayscore = games[col].visitor_g;
    ...
    std::string abbrev_update = "UPDATE SRS as r SET team_abbreviation = (SELECT abbreviation FROM pro_api_teams AS t WHERE r.team_id = t.bball_ref_id)";
    // Commit the transaction
